...
export class CandleStickChart {
  ...
  reSize() {
    ...
    this.labelDayInterval = 0;
    this.labelMonthInterval = 0;
    this.labelYearInterval = 0;

    const min_cols = Math.round(this.width / 150);
    this.labelInterval = Math.ceil(this.intervalSeconds / min_cols);
    console.log(this.labelInterval);
  ...
  zoom(e) {
    e.preventDefault();
    const n = Math.floor(this.visible_candles.length / 100);
  ...
  createGrid() {
    ...
    for (let i = this.visible_candles.length - 1; i >= 0; i--) {
      const candle = this.visible_candles[i];
      let text = i;
