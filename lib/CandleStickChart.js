...
export class CandleStickChart {
  ...
  draw() {
    ...
    this.createChart();
    // TODO don't re-calculate the grid unless reSize has been called.
    this.createGrid();
  ...
  reSize() {
    ...
    const nDays = this.visible_candles.length;
    const min_cols = Math.round(this.width / 200);
    this.dateInterval = Math.ceil(nDays / min_cols);
  ...
  zoom(e) {
    e.preventDefault();
    const n = Math.floor(this.visible_candles.length / 50);
  ...
  volumeToCoordinate(volume) {
    ...
    if (y > this.height) console.log(volume, height, y);
    return y;
  }
  ...
  createGrid() {
    ...
    let prev_day = this.visible_candles[0].timestamp.getDay();
    let prev_month = this.visible_candles[0].timestamp.getMonth();
    let prev_year = this.visible_candles[0].timestamp.getYear();

    for (let i = 0; i < this.visible_candles.length; i++) {
      if (i % this.dateInterval == 0) {
        const candle = this.visible_candles[i];
        const x = this.candleToY(candle);
        console.log(candle, x);
        this.drawLine(
          {
            x1: x,
            y1: 0,
        ...
        const day = candle.timestamp.toLocaleString("default", {
          day: "numeric",
        });
        const month = candle.timestamp.toLocaleString("default", {
          month: "long",
        });
        const year = candle.timestamp.getYear();
        let text = month;
  ...
  createCandleSticks(data) {
    ...
    this.interval =
      this.visible_candles[0].timestamp - this.visible_candles[1].timestamp;
  }
  ...
  drawCandleStick(candlestick) {
    ...
    const x = this.candleToY(candlestick) - width / 2;
    ...
    const y_high = close > open ? this.priceToX(close) : this.priceToX(open);

    const y_length = Math.abs(this.priceToX(open) - this.priceToX(close));
    ...
    this.drawLine(
      {
        x1: x + width * 0.5,
        y1: this.priceToX(high) + 0.25,
        x2: x + width * 0.5,
        y2: this.priceToX(low) - 0.25,
      },
      color
    );
  }
